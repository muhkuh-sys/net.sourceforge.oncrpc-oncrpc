	...
	bool_t eor;
{
	...
	register u_long len = (ptrdiff_t)(rstrm->out_finger) -
		(ptrdiff_t)(rstrm->frag_header) - sizeof(u_long);

	*(rstrm->frag_header) = htonl(len | eormask);
	len = (ptrdiff_t)(rstrm->out_finger) - (ptrdiff_t)(rstrm->out_base);
	...
	register RECSTREAM *rstrm;
{
	...
	where = rstrm->in_base;
	i = (ptrdiff_t)rstrm->in_boundry % BYTES_PER_XDR_UNIT;
	where += i;
	len = rstrm->in_size - i;
	...
	register int len;
{
	...
	while (len > 0) {
		current = (ptrdiff_t)rstrm->in_boundry - (ptrdiff_t)rstrm->in_finger;
		if (current == 0) {
	...
	int (*writeit)();  /* like write, but pass it a tcp_handle, not sock */
{
	...
	for (rstrm->out_base = rstrm->the_buffer;
		(ptrdiff_t)rstrm->out_base % BYTES_PER_XDR_UNIT != 0;
		rstrm->out_base++);
	...
	long *lp;
{
	...
	if ((rstrm->fbtbc >= sizeof(long)) &&
		(((ptrdiff_t)rstrm->in_boundry - (ptrdiff_t)buflp) >= sizeof(long))) {
	...
	register u_int len;
{
	...
	while (len > 0) {
		current = (ptrdiff_t)rstrm->out_boundry - (ptrdiff_t)rstrm->out_finger;
		current = (len < (u_int)current) ? len : current;
	...
	register XDR *xdrs;
{
	...
	pos = lseek((ptrdiff_t)rstrm->tcp_handle, (long) 0, 1);
	if (pos != -1)
		switch (xdrs->x_op) {
	...
	bool_t sendnow;
{
	...
	if (sendnow || rstrm->frag_sent ||
		((ptrdiff_t)rstrm->out_finger + sizeof(u_long) >=
